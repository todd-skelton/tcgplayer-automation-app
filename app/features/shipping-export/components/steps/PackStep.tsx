...
export function PackStep({
  ...
  // Unique shipment references (one card per merged shipment)
  const shipmentReferences = Object.keys(shipmentToOrderMap).length > 0
  ...
  // Find the primary order for this reference
  const currentOrder = sourceOrders.find((o) => o["Order #"] === currentReference) ?? null;

  // All orders merged into this shipment (for combined shipments)
  ...
  const purchaseEntry = currentReference ? outboundPurchaseResultsByReference[currentReference] : null;
  const isPacked = currentReference ? packedOrderNumbers.has(currentReference) : false;
  const packedCount = packedOrderNumbers.size;
  const progress = totalShipments > 0 ? (packedCount / totalShipments) * 100 : 0;

  // All line items across merged orders for this shipment
  const allLineItems = mergedOrders.flatMap((o) => o.products ?? []);
  const hasLineItems = allLineItems.length > 0;

  // Aggregate line items across merged orders
    ...
    <Stack spacing={3}>
      <Stack direction="row" spacing={2} alignItems="center" justifyContent="space-between">
        <Stack direction="row" spacing={1} alignItems="center">
          ...
          <IconButton
            onClick={() => setCurrentIndex((i) => Math.min(totalShipments - 1, i + 1))}
            ...
            <Stack spacing={1.5}>
              ...
              <Stack direction="row" spacing={2}>
                ...
                <Box>
                  ...
                  <Typography variant="body2">
                    ${mergedOrders.reduce((s, o) => s + o["Value Of Products"], 0).toFixed(2)}
                  </Typography>
                </Box>
                <Box>
                  <Typography variant="body2" color="text.secondary">Method</Typography>
                  <Typography variant="body2">{currentOrder["Shipping Method"]}</Typography>
                </Box>
              </Stack>

              {purchaseEntry && (
                <Box>
                  <Typography variant="body2" color="text.secondary">Postage</Typography>
                  <Stack direction="row" spacing={1} alignItems="center">
                    <Chip
              ...
              <FormControlLabel
                control={
                  <Checkbox
                    ...
                    onChange={(_, checked) => {
                      if (currentReference) {
                        onOrderPacked(currentReference, checked);
                      }
                    }}
                    color="success"
                  />
      ...
      {!currentOrder && (
        <Alert severity="warning">No order found for this shipment reference.</Alert>
      )}
