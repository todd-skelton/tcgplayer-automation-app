...
Each worker cycle reads at most one page and 25 details, with five detail requests at once and a ten-second deadline per request. Requests disable transport retry so a cycle cannot expand its request budget. The run saves its offset, unique order identities, observed bounds, failed detail order numbers, and current total. A later cycle retries detail gaps before advancing, and every resumed page overlaps the preceding page to tolerate movement in offset pagination. Empty or repeated pages, failed details, deadline failures, and budget stops leave explicit incomplete coverage; missing orders never imply cancellation or deletion.

Only one leased worker may advance a seller's run. API observations, checkpoints, and completion are fenced by its claim token. A stopped worker's claim expires after two minutes and the next cycle resumes its saved checkpoint. A new run starts 15 minutes after completion and overlaps the whole currently supported moving window, allowing late orders and changed lifecycle/refund/line observations to create new revisions. Per-order checkpoint identities are removed on completion and only the latest 30 completed runs per seller are retained. Orders that age out of that window retain their last observation, but later changes to them are a declared coverage gap unless supplied by file import.
