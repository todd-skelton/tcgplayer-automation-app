...
Cancellations follow two assumptions (see `docs/strategy-assumptions.md`): a canceled order that never shipped never consumed stock and is recorded as `canceled_unfulfilled` with no allocation, because the seller puts it straight back in the portal; a canceled order that shipped is a return that re-enters as new intake in a later batch, so its allocation stands. A recorded disposition takes precedence over either assumption. `unfulfilled_cancellation` requires a fully matched, canceled, never-shipped line. `physical_restock` requires the exact current source allocation keys, receipt IDs, quantities, and a new availability time. If a later provider revision reduces that line, FIFO holds until `supersede_disposition` proves that the reduction accounts for the return. The correction retires the returned quantity from its exact prior supply segment and retains the returned segment at its recorded availability time, including nested returns. Acknowledging an opening observation difference does not change stock or clear its FIFO hold.
...
Pre-cutoff canceled orders follow the same cancellation assumptions as forward orders. Orders before `covered_from` remain `excluded_pre_cutoff`.
